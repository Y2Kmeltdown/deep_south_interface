...
module altera_hbm_tg_axi_axi_if # (
   ...
   reg [RAND_ADDR_WIDTH-1:0] awaddr_seq_rd_or_wr, araddr_seq_rd_or_wr;
   reg [RAND_ADDR_WIDTH-1:0] virt_addr_seq_rd_or_wr;
   assign rd_valid = arvalid && arready;
   assign wr_valid = awvalid && awready;
   	always @ (posedge clk) begin
	   if (rst) begin
			  awaddr_seq_rd_or_wr    <= 0;
			  araddr_seq_rd_or_wr    <= 0;
			  virt_addr_seq_rd_or_wr <= 0;
	   end else begin
		    if (wr_valid) awaddr_seq_rd_or_wr <= awaddr_seq_rd_or_wr + SEQ_LEN_1;
		    if (rd_valid) araddr_seq_rd_or_wr <= araddr_seq_rd_or_wr + SEQ_LEN_1;
		    if (wlast & wready & wvalid)
		                  virt_addr_seq_rd_or_wr <= virt_addr_seq_rd_or_wr + SEQ_LEN_1;
	   end
	  end
  ...
  */
   ...
   assign awaddr                     =  (TG_USE_EFFICIENCY_PATTERN)? (
                                        ...
                                        (SEQUENTIAL_RANDOM)?     {awaddr_seq_rd_or_wr, {ADDR_PAD{1'b0}}}:
                                                                 {write_addr         , {($size(awaddr)-$size(write_addr)){1'b0}}} ))):
   ...
   assign araddr                     = (TG_USE_EFFICIENCY_PATTERN)? (
   					(STRESS_LFSR)?             {pat_rd_addr        , {ADDR_PAD{1'b0}}}:(
                                       (SEQUENTIAL_RANDOM)?  {araddr_seq_rd_or_wr, {ADDR_PAD{1'b0}}}:
                                                             {read_addr          , {($size(araddr)-$size(read_addr)){1'b0}}} )):
