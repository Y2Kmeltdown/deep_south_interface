...
if { ! [timing_netlist_exist] } {
   ...
   source "$script_dir/hbm_bottom_example_design_altera_abstract_uib_191_b3v3bdi_report_timing_core.tcl"
   if { ! [timing_netlist_exist] } {
...
if [ info exists hbm_db ] {
   unset hbm_db
}
hbm_bottom_example_design_altera_abstract_uib_191_b3v3bdi_initialize_hbm_db hbm_db var

# If multiple instances of this core are present in the
# design they will all be analyzed through the
# following loop
set instances [ array names hbm_db ]
set inst_id 0
foreach inst $instances {
   ...
   if {[llength $instances] <= 1} {
      set fbasename "${::GLOBAL_hbm_bottom_example_design_altera_abstract_uib_191_b3v3bdi_corename}"
   } else {
      set fbasename "${::GLOBAL_hbm_bottom_example_design_altera_abstract_uib_191_b3v3bdi_corename}_${inst_id}"
   }
   ...
   hbm_bottom_example_design_altera_abstract_uib_191_b3v3bdi_perform_core_analysis $opcname $inst pins var summary

   #######################################
   # Print out the Summary Panel for this instance   

   set summary [lsort -command hbm_bottom_example_design_altera_abstract_uib_191_b3v3bdi_sort_proc $summary]

   set f -1
   set fname "${fbasename}_summary.csv"

   if { [hbm_bottom_example_design_altera_abstract_uib_191_b3v3bdi_get_operating_conditions_number] == 0 } {
      set f [open $fname w]

      puts $f "Core: ${::GLOBAL_hbm_bottom_example_design_altera_abstract_uib_191_b3v3bdi_corename} - Instance: $inst"
   ...
   post_message -type info "Core: ${::GLOBAL_hbm_bottom_example_design_altera_abstract_uib_191_b3v3bdi_corename} - Instance: $inst"
   ...
   foreach summary_line $summary {
      ...
      if {$su != "--"} {
         set su [ hbm_bottom_example_design_altera_abstract_uib_191_b3v3bdi_round_3dp $su]
      }
      if {$hold != "--"} {
         set hold [ hbm_bottom_example_design_altera_abstract_uib_191_b3v3bdi_round_3dp $hold]
      }
   ...
   close $f
   if {$total_failures > 0} {
      post_message -type critical_warning "HBM Timing requirements not met"
...
if {$curr_speedgrade != $effective_target_speedgrade} {
   post_message -type critical_warning "This HBM Interface IP core was generated for a speed grade $effective_target_speedgrade device, but the speed grade of $curr_part is $curr_speedgrade. You should regenerate the IP core to match the target device to avoid hardware issue."
}
