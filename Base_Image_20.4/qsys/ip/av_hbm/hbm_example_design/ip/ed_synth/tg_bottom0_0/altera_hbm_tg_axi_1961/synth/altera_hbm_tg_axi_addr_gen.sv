...
module altera_hbm_tg_axi_addr_gen # (
   ...
   output logic [AMM_BURSTCOUNT_WIDTH-1:0] burstcount_out,
   ...
   generate
     if(BURST_LEN_EXTEND_EN) begin  
	   ...
	   altera_hbm_tg_axi_rand_num_gen # (
              ...
              .enable            (!start & enable),
              .ready             (),
              .rand_num          (burstcount_out)
           );
	end
     end else begin
        assign burstcount_out = (BURST_LEN == 2) ? {{(AMM_BURSTCOUNT_WIDTH-2){1'b0}},2'b10} : {{(AMM_BURSTCOUNT_WIDTH-1){1'b0}},1'b1};
     end
