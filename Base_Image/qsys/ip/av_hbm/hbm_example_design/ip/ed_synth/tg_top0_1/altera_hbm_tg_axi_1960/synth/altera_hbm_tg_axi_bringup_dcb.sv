...
module altera_hbm_tg_axi_bringup_dcb #(
  ...
  parameter BURST_COUNT_MODE      = "RAND"
   ...
   localparam NUM_DRIVER_LOOP  = (TG_TEST_DURATION == "INFINITE") ? 0    :
                                 (TG_TEST_DURATION == "MEDIUM")   ? (BURST_LEN_EXTEND_EN? 1:1024) :
  ...
  `endif
   ...
   localparam TG_MAX_BURSTCOUNT         = 2**BURSTCOUNT_WIDTH - 1;
   ...
   altera_hbm_tg_axi_rw_stage # (
      ...
      .BYTE_EN_PATTERN_LENGTH       (BYTE_EN_PATTERN_LENGTH),
      .BURSTCOUNT_WIDTH             (BURSTCOUNT_WIDTH),
      .BURST_COUNT_DIVISIBLE_BY     (BURST_LEN_EXTEND_EN? (2**$clog2(MAX_BURST_COUNT)) : BURST_COUNT_DIVISIBLE_BY),
      .MEM_ADDR_WIDTH               (MEM_ADDR_WIDTH),
      .SEED_OFFSET                  (SEED_OFFSET),
      .BURST_LEN                    (BURST_LEN),
      .PORT_TG_CFG_ADDRESS_WIDTH    (PORT_TG_CFG_ADDRESS_WIDTH),
