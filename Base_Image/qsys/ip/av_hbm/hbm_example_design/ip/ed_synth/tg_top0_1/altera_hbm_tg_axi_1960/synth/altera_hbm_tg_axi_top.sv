...
module altera_hbm_tg_axi_top # (
   ...
   localparam INT_WORD_ADDR_WIDTH = (!TG_USE_EFFICIENCY_PATTERN && ADDR_MASK_VALUE && !USE_SIMPLE_TG)? (WORD_ADDR_WIDTH - 2):WORD_ADDR_WIDTH;   
   localparam NUM_OF_PATTERNS = 1;
   localparam STATIC_RATIO_1  = 0;
   localparam NUM_EFFECTIVE_ADDRESSES = 2**(INT_WORD_ADDR_WIDTH-((BURST_LEN == 2)? 1: 0));
...
*/
   ...
   localparam INT_AXI_AWADDR_WIDTH = (!TG_USE_EFFICIENCY_PATTERN && ADDR_MASK_VALUE && !USE_SIMPLE_TG)? (PORT_AXI_AWADDR_WIDTH - 2):PORT_AXI_AWADDR_WIDTH;
   localparam INT_AXI_ARADDR_WIDTH = (!TG_USE_EFFICIENCY_PATTERN && ADDR_MASK_VALUE && !USE_SIMPLE_TG)? (PORT_AXI_ARADDR_WIDTH - 2):PORT_AXI_ARADDR_WIDTH;


   wire [PORT_AXI_AWID_WIDTH-1:0]      awid_tg;
   wire [INT_AXI_AWADDR_WIDTH-1:0]     awaddr_tg;
   wire [INT_AXI_AWADDR_WIDTH-1:0]     awaddr_pre_mask;   
   wire [PORT_AXI_AWLEN_WIDTH-1:0]     awlen_tg;
   wire [PORT_AXI_AWLEN_WIDTH-1:0]     awlen_tf_gen;
   ...
   wire [INT_AXI_ARADDR_WIDTH-1:0]     araddr_tg;
   wire [INT_AXI_ARADDR_WIDTH-1:0]     araddr_pre_mask;   
   wire [PORT_AXI_ARLEN_WIDTH-1:0]     arlen_tg;
   wire [PORT_AXI_ARLEN_WIDTH-1:0]     arlen_tf_gen;
   ...
   wire [INT_AXI_AWADDR_WIDTH-1:0]    awaddr_pre_bit_swap;
   wire [INT_AXI_ARADDR_WIDTH-1:0]    araddr_pre_bit_swap;

   
   localparam [1:0] ADDR_MASK_BIT_VALUE =  (ADDR_MASK_VALUE == 1)? 2'b00: ((ADDR_MASK_VALUE == 2)? 2'b01: ((ADDR_MASK_VALUE == 3)? 2'b10: ((ADDR_MASK_VALUE == 4)? 2'b11: 2'b00))); 
   localparam ADDR_MASK_BIT = 26;  
   generate
      if((ADDR_MASK_BIT == INT_AXI_ARADDR_WIDTH-2) && ADDR_MASK_VALUE) begin
         assign araddr = {ADDR_MASK_BIT_VALUE[1:0], araddr_pre_mask[ADDR_MASK_BIT-1:0]};
         assign awaddr = {ADDR_MASK_BIT_VALUE[1:0], awaddr_pre_mask[ADDR_MASK_BIT-1:0]};	 
      end 
      else if (!TG_USE_EFFICIENCY_PATTERN && !USE_SIMPLE_TG && ADDR_MASK_VALUE) begin
         assign araddr = {araddr_pre_mask[INT_AXI_ARADDR_WIDTH-1:ADDR_MASK_BIT], ADDR_MASK_BIT_VALUE[1:0], araddr_pre_mask[ADDR_MASK_BIT-1:0]};
         assign awaddr = {awaddr_pre_mask[INT_AXI_AWADDR_WIDTH-1:ADDR_MASK_BIT], ADDR_MASK_BIT_VALUE[1:0], awaddr_pre_mask[ADDR_MASK_BIT-1:0]}; 
      end
      else if (TG_USE_EFFICIENCY_PATTERN && ADDR_MASK_VALUE) begin
         assign araddr = {araddr_pre_mask[INT_AXI_ARADDR_WIDTH-1:ADDR_MASK_BIT+2], ADDR_MASK_BIT_VALUE[1:0], araddr_pre_mask[ADDR_MASK_BIT-1:0]};
         assign awaddr = {awaddr_pre_mask[INT_AXI_AWADDR_WIDTH-1:ADDR_MASK_BIT+2], ADDR_MASK_BIT_VALUE[1:0], awaddr_pre_mask[ADDR_MASK_BIT-1:0]}; 
      end 
      else begin	 
         assign araddr = araddr_pre_mask;
         assign awaddr = awaddr_pre_mask;
      end   
   endgenerate

   altera_hbm_tg_axi_backpressure #(
      .PORT_AXI_AWID_WIDTH              (PORT_AXI_AWID_WIDTH),
      .PORT_AXI_AWADDR_WIDTH            (INT_AXI_AWADDR_WIDTH),
      ...
      .PORT_AXI_ARADDR_WIDTH            (INT_AXI_ARADDR_WIDTH),
      ...
      .awid_hbm           (awid),
      .awaddr_hbm         (awaddr_pre_bit_swap),
      .awlen_hbm          (awlen),
      .awsize_hbm         (awsize),
      .awburst_hbm        (awburst),
      .awprot_hbm         (awprot),
      ...
      .bready_hbm         (bready),
      .arid_hbm           (arid),
      .araddr_hbm         (araddr_pre_bit_swap),
      .arlen_hbm          (arlen),
      .arsize_hbm         (arsize),
      .arburst_hbm        (arburst),
      .arprot_hbm         (arprot),
   ...
   localparam BURSTCOUNT_WIDTH         = BURST_LEN_EXTEND_EN? 9 : 4;
   ...
   wire [INT_WORD_ADDR_WIDTH-1:0]       ast_exp_data_readaddr;
   ...
   wire [INT_WORD_ADDR_WIDTH-1:0] first_fail_addr;
   ...
   if (USE_SIMPLE_TG) begin
        altera_hbm_tg_axi_driver_simple #(
            ...
            .MEM_ADDR_WIDTH                 (INT_WORD_ADDR_WIDTH),
            .WORD_ADDRESS_DIVISIBLE_BY      (WORD_ADDRESS_DIVISIBLE_BY),
            .BURST_LEN                      (BURST_LEN_EXTEND_EN? MAX_BURST_COUNT:BURST_LEN),
            .PORT_AXI_AWID_WIDTH            (PORT_AXI_AWID_WIDTH),
            .PORT_AXI_AWADDR_WIDTH          (INT_AXI_AWADDR_WIDTH),
            .PORT_AXI_AWQOS_WIDTH           (PORT_AXI_AWQOS_WIDTH),
            .PORT_AXI_AWUSER_AP_WIDTH       (PORT_AXI_AWUSER_AP_WIDTH),
   ...
   traffic_gen_inst(
      ...
      .pass                       (traffic_gen_pass),
      .first_fail_addr            ({{(64-INT_WORD_ADDR_WIDTH){1'b0}}, first_fail_addr}),
      ...
      altera_hbm_tg_axi_status_checker # (
         ...
         .ADDR_WIDTH                   (INT_WORD_ADDR_WIDTH),
         .OP_COUNT_WIDTH               (TOTAL_OP_COUNT_WIDTH),
   ...
   generate
      if (PROTOCOL_ENUM == "PROTOCOL_DDRT") begin
         assign arburst_tg = '0;
         assign arlen_tg = '0;
         assign arprot_tg = '0;
         assign arsize_tg = $clog2(PORT_AXI_WSTRB_WIDTH);
         assign awburst_tg = '0;
         assign awlen_tg = '0;
         assign awprot_tg = '0;
         assign awaddr_pre_mask = {awaddr_pre_bit_swap[PORT_AXI_AWADDR_WIDTH-1:23],1'b1,awaddr_pre_bit_swap[22:1]};
         assign araddr_pre_mask = {araddr_pre_bit_swap[PORT_AXI_AWADDR_WIDTH-1:23],1'b1,araddr_pre_bit_swap[22:1]};
         assign awsize_tg = $clog2(PORT_AXI_WSTRB_WIDTH);
      end else begin
         assign arburst_tg = '0;
         assign arprot_tg = '0;
         assign awburst_tg = '0;
         assign awprot_tg = '0;
         assign {awsize_tg , arsize_tg} = (PORT_AXI_WDATA_WIDTH == 128) ? {3'b100,3'b100} : {3'b101,3'b101};
         assign arlen_tg = arlen_tf_gen;
	 assign awlen_tg = awlen_tf_gen;
         assign awaddr_pre_mask = awaddr_pre_bit_swap;
         assign araddr_pre_mask = araddr_pre_bit_swap;
      end
   endgenerate
