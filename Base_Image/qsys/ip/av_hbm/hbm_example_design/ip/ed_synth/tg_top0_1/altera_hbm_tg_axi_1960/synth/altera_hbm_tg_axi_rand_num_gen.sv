...
module altera_hbm_tg_axi_max_min_burst_gen # (
   parameter NUM_WIDTH                    = "",
   parameter NUM_MIN                      = "",
   parameter NUM_MAX                      = ""
) (
   ...
   // Burst length generator output
   output logic [NUM_WIDTH-1:0]   burstcount_out
);
   timeunit 1ns;
   timeprecision 1ps;
   ...
   logic toggle;
      ...
      begin
         if (rst) begin
            burstcount_out <= NUM_MIN[NUM_WIDTH-1:0];
            toggle         <= 1'b0;
         end else begin
            toggle         <= enable? ~toggle : toggle;
            burstcount_out <= enable? (toggle? NUM_MIN[NUM_WIDTH-1:0] : NUM_MAX[NUM_WIDTH-1:0]) : burstcount_out;
   ...
   initial
   begin
      assert (NUM_MAX >= NUM_MIN) else $error ("Invalid random number range");
      assert (NUM_MAX < 2**NUM_WIDTH) else $error ("Invalid random number width");
   end
endmodule

module altera_hbm_tg_axi_seq_burst_gen # (
   parameter NUM_WIDTH                    = "",
   parameter NUM_MIN                      = "",
   parameter NUM_MAX                      = ""
) (
   ...
   timeprecision 1ps;
      ...
      begin
         if (rst) begin
            ...
            if(enable) begin
	       if(burstcount_out == NUM_MAX[NUM_WIDTH-1:0]) begin
	          burstcount_out <= NUM_MIN[NUM_WIDTH-1:0];
	       end else begin
	          burstcount_out <= burstcount_out + 1'b1;
	       end
	    end
         end
      end
