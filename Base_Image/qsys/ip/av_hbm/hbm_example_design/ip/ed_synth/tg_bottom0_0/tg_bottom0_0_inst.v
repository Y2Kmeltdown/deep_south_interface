	tg_bottom0_0 u0 (
		.wmc_clk_in          (_connected_to_wmc_clk_in_),          //   input,    width = 1,  wmc_clk_in.clk
		.wmcrst_n_in         (_connected_to_wmcrst_n_in_),         //   input,    width = 1, wmcrst_n_in.reset_n
		.ninit_done          (_connected_to_ninit_done_),          //   input,    width = 1,  ninit_done.ninit_done
		.awid                (_connected_to_awid_),                //  output,    width = 9,         axi.awid
		.awaddr              (_connected_to_awaddr_),              //  output,   width = 29,            .awaddr
		.awlen               (_connected_to_awlen_),               //  output,    width = 8,            .awlen
		.awsize              (_connected_to_awsize_),              //  output,    width = 3,            .awsize
		.awburst             (_connected_to_awburst_),             //  output,    width = 2,            .awburst
		.awprot              (_connected_to_awprot_),              //  output,    width = 3,            .awprot
		.awqos               (_connected_to_awqos_),               //  output,    width = 4,            .awqos
		.awuser_ap           (_connected_to_awuser_ap_),           //  output,    width = 1,            .awuser
		.awvalid             (_connected_to_awvalid_),             //  output,    width = 1,            .awvalid
		.awready             (_connected_to_awready_),             //   input,    width = 1,            .awready
		.wdata               (_connected_to_wdata_),               //  output,  width = 256,            .wdata
		.wstrb               (_connected_to_wstrb_),               //  output,   width = 32,            .wstrb
		.wlast               (_connected_to_wlast_),               //  output,    width = 1,            .wlast
		.wvalid              (_connected_to_wvalid_),              //  output,    width = 1,            .wvalid
		.wready              (_connected_to_wready_),              //   input,    width = 1,            .wready
		.bid                 (_connected_to_bid_),                 //   input,    width = 9,            .bid
		.bresp               (_connected_to_bresp_),               //   input,    width = 2,            .bresp
		.bvalid              (_connected_to_bvalid_),              //   input,    width = 1,            .bvalid
		.bready              (_connected_to_bready_),              //  output,    width = 1,            .bready
		.arid                (_connected_to_arid_),                //  output,    width = 9,            .arid
		.araddr              (_connected_to_araddr_),              //  output,   width = 29,            .araddr
		.arlen               (_connected_to_arlen_),               //  output,    width = 8,            .arlen
		.arsize              (_connected_to_arsize_),              //  output,    width = 3,            .arsize
		.arburst             (_connected_to_arburst_),             //  output,    width = 2,            .arburst
		.arprot              (_connected_to_arprot_),              //  output,    width = 3,            .arprot
		.arqos               (_connected_to_arqos_),               //  output,    width = 4,            .arqos
		.aruser_ap           (_connected_to_aruser_ap_),           //  output,    width = 1,            .aruser
		.arvalid             (_connected_to_arvalid_),             //  output,    width = 1,            .arvalid
		.arready             (_connected_to_arready_),             //   input,    width = 1,            .arready
		.rid                 (_connected_to_rid_),                 //   input,    width = 9,            .rid
		.rdata               (_connected_to_rdata_),               //   input,  width = 256,            .rdata
		.rresp               (_connected_to_rresp_),               //   input,    width = 2,            .rresp
		.rlast               (_connected_to_rlast_),               //   input,    width = 1,            .rlast
		.rvalid              (_connected_to_rvalid_),              //   input,    width = 1,            .rvalid
		.rready              (_connected_to_rready_),              //  output,    width = 1,            .rready
		.ruser_err_dbe       (_connected_to_ruser_err_dbe_),       //   input,    width = 1,   axi_extra.ruser_err_dbe
		.ruser_data          (_connected_to_ruser_data_),          //   input,   width = 32,            .ruser_data
		.wuser_data          (_connected_to_wuser_data_),          //  output,   width = 32,            .wuser_data
		.wuser_strb          (_connected_to_wuser_strb_),          //  output,    width = 4,            .wuser_strb
		.traffic_gen_pass    (_connected_to_traffic_gen_pass_),    //  output,    width = 1,   tg_status.traffic_gen_pass
		.traffic_gen_fail    (_connected_to_traffic_gen_fail_),    //  output,    width = 1,            .traffic_gen_fail
		.traffic_gen_timeout (_connected_to_traffic_gen_timeout_), //  output,    width = 1,            .traffic_gen_timeout
		.ur_paddr            (_connected_to_ur_paddr_),            //  output,   width = 16,         apb.ur_paddr
		.ur_psel             (_connected_to_ur_psel_),             //  output,    width = 1,            .ur_psel
		.ur_penable          (_connected_to_ur_penable_),          //  output,    width = 1,            .ur_penable
		.ur_pwrite           (_connected_to_ur_pwrite_),           //  output,    width = 1,            .ur_pwrite
		.ur_pwdata           (_connected_to_ur_pwdata_),           //  output,   width = 16,            .ur_pwdata
		.ur_pstrb            (_connected_to_ur_pstrb_),            //  output,    width = 2,            .ur_pstrb
		.ur_prready          (_connected_to_ur_prready_),          //   input,    width = 1,            .ur_prready
		.ur_prdata           (_connected_to_ur_prdata_)            //   input,   width = 16,            .ur_prdata
	);

