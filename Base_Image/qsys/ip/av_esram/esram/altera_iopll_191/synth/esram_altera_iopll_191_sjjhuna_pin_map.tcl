...
# esram_altera_iopll_191_sjjhuna.sdc scripts. 
#
# These routines are only meant to support the SDC. 
# Trying to using them in a different context can have unexpected 
# results.

set ::GLOBAL_top_esram_altera_iopll_191_sjjhuna_corename_debug 0

set script_dir [file dirname [info script]]

source [file join $script_dir esram_altera_iopll_191_sjjhuna_parameters.tcl]
...
set ::GLOBAL_top_esram_altera_iopll_191_sjjhuna_corename_disable_warnings [get_warnings_disabled]
...
# ----------------------------------------------------------------
    ...
    if {$msg_type == "debug"} {
        if {$::GLOBAL_top_esram_altera_iopll_191_sjjhuna_corename_debug} {
            puts $msg
        }
    } else {
        if {$msg_context == "all"} {
...
# ----------------------------------------------------------------
	...
	global ::GLOBAL_top_esram_altera_iopll_191_sjjhuna_corename

	ai_post_message info "Initializing PLL database for CORE $::GLOBAL_top_esram_altera_iopll_191_sjjhuna_corename"
	set instance_list [ai_get_core_instance_list $::GLOBAL_top_esram_altera_iopll_191_sjjhuna_corename]

	foreach instname $instance_list {
		ai_post_message info "Finding port-to-pin mapping for CORE: $::GLOBAL_top_esram_altera_iopll_191_sjjhuna_corename INSTANCE: $instname"
...
# ----------------------------------------------------------------
	...
	if {[ llength $instance_list ] == 0} {
        ...
        if {!$::GLOBAL_top_esram_altera_iopll_191_sjjhuna_corename_disable_warnings} {
...
# ----------------------------------------------------------------
    ...
    set base_clock_data_dict $::GLOBAL_top_esram_altera_iopll_191_sjjhuna_base_clock_data
    set gen_clock_data_dict $::GLOBAL_top_esram_altera_iopll_191_sjjhuna_gen_clock_data
