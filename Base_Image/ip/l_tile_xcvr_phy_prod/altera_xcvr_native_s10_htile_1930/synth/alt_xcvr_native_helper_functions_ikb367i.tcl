...
# ---                                                                    --- #
# -------------------------------------------------------------------------- #
proc native_initialize_db_ikb367i { native_db } {
  ...
  native_get_pins_ikb367i $all_pins
...
# ---                                                                    --- #
# -------------------------------------------------------------------------- #
proc native_get_pins_ikb367i { all_pins } {
...
# ---                                                                          --- #
# -------------------------------------------------------------------------------- #
proc native_prepare_to_create_clocks_all_ch_ikb367i { instance num_channels mode mode_clks profile_cnt profile alt_xcvr_native_s10_pins clk_freq_dict multiply_factor_dict divide_factor_dict all_profile_clocks_names } {
  global ::native_debug
    ...
    if { [dict exists $alt_xcvr_native_s10_pins $instance $clk_group] } {
      ...
      if { [llength $clk_pins] > 0 } { # Check to see if the corresponding pins exists 
        ...
        if { $mode == "tx_source_clks" || $mode == "rx_source_clks"} {
          ...
          lappend list_of_clk_names [native_create_clocks_all_ch_ikb367i $instance $clk_group $num_channels $clk_freq $clk_pins $profile_cnt $profile]
          ...
          # Create clks for all channels for a clk group in mode clk
          lappend list_of_clk_names [native_create_clocks_all_ch_ikb367i $instance $clk_group $num_channels $clk_freq $clk_pins $profile_cnt $profile $source_nodes $master_clocks $multiply_factor $divide_factor]
        }
      }
...
# ---                                                                       --- #
# ----------------------------------------------------------------------------- #
proc native_create_clocks_all_ch_ikb367i { instance clk_group num_channels freq clk_list profile_cnt profile args } {
  global ::native_debug
        ...
        foreach_in_collection matching_clk $matching_clocks_list {
          ...
          # Clock was declared, but not defined, so we need to create the clock still (i.e. "declare_clock" command was used)
            ...
            if { $args != "" } {
              ...
              native_create_clock_ikb367i $clk_group $clock_name $freq $no_inst_clk_node $channel $no_inst_clk_source_node $clk_master_clock $multiply_factor $divide_factor

            } else {

              # Call procedure to create source clock for given clock node 
              native_create_clock_ikb367i $clk_group $clock_name $freq $no_inst_clk_node $channel
            }
          }
        }; #foreach_in_collection matching_clk matching_clocks_list
...
# ---                                                                       --- #
# ----------------------------------------------------------------------------- #
proc native_create_clock_ikb367i { clk_group clock_name freq clk_node channel args } {
  global ::native_debug
  global ::alt_xcvr_native_s10_target_clock_list_dict
...
# ---                                                                       --- #
# ----------------------------------------------------------------------------- #
proc native_check_special_min_pulse_ikb367i { clock_name } {
  global ::native_debug
  set pass 1  
