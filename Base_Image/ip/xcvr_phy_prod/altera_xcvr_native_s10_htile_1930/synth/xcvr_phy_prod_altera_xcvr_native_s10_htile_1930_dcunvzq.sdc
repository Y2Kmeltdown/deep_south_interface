...
# ---      alt_xcvr_native_helper_functions_dcunvzq.tcl        --- #
# -                                                              - # 
# ---------------------------------------------------------------- #

set script_dir [file dirname [info script]] 
set split_qsys_output_name [split xcvr_phy_prod_altera_xcvr_native_s10_htile_1930_dcunvzq "_"]
...
source "${script_dir}/${top_inst_name}_ip_parameters_dcunvzq.tcl"
source "${script_dir}/alt_xcvr_native_helper_functions_dcunvzq.tcl"

# Debug switch. Change to 1 in alt_xcvr_native_helper_functions_dcunvzq.tcl to get more run-time debug information
...
# -                                                              - #
# ---------------------------------------------------------------- #
native_initialize_db_dcunvzq nativedb_dcunvzq

# ---------------------------------------------------------------- #
# --- Set all the instances of this core                       --- #
# ---------------------------------------------------------------- #
set alt_xcvr_native_s10_instances [ dict keys $nativedb_dcunvzq ]
...
foreach inst $alt_xcvr_native_s10_instances {
  ...
  dict set alt_xcvr_native_s10_pins $inst [dict get $nativedb_dcunvzq $inst]
  ...
  for {set i 0} {$i < $profile_cnt} {incr i} {
    ...
    dict for {mode mode_clks} $profile_clocks {
      ...
      set list_of_clk_names [native_prepare_to_create_clocks_all_ch_dcunvzq $inst $num_channels $mode $mode_clks $profile_cnt $i $alt_xcvr_native_s10_pins $freq $multiply_factor_dict $divide_factor_dict $all_profile_clocks_names]
      ...
      foreach_in_collection fanin_node $tx_coreclkin_fanin_col {
        set fanin_node_name [get_node_info -name $fanin_node]
         ...
         set_active_clocks [remove_from_collection [get_active_clocks] [get_clocks $fanin_clock_name]]
          ...
          add_ddr_report_command "native_check_special_min_pulse_dcunvzq $fanin_clock_name"
