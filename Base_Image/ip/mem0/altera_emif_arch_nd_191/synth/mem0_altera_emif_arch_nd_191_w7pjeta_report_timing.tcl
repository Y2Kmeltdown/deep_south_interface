...
proc get_speedgrade_from_opn {part} {
   ...
   if {$power_model == "Smart-VID"} {
      set power_model "V"
   } elseif {$power_model == "Low"} {
      set power_model "L"
   } elseif {$power_model == "{Ultra Extreme Low}"} {
      set power_model "Y"
   } elseif {$power_model == "{Extreme Low}"} {
      set power_model "X"
   } elseif {$power_model == "{Wider Range Smart-VID}"} {
      set power_model "W"
   } else {
      return ""
   }
...
if { ! [timing_netlist_exist] } {
   ...
   source "$script_dir/mem0_altera_emif_arch_nd_191_w7pjeta_report_timing_core.tcl"
   if { ! [timing_netlist_exist] } {
...
mem0_altera_emif_arch_nd_191_w7pjeta_initialize_ddr_db ddr_db var
...
foreach inst $instances {
   ...
   if {[llength $instances] <= 1} {
      set fbasename "${::GLOBAL_mem0_altera_emif_arch_nd_191_w7pjeta_corename}"
   } else {
      set fbasename "${::GLOBAL_mem0_altera_emif_arch_nd_191_w7pjeta_corename}_${inst_id}"
   }
   ...
   mem0_altera_emif_arch_nd_191_w7pjeta_perform_read_capture_analysis $opcname $inst pins var summary

   #######################################
   # Write Analysis

   mem0_altera_emif_arch_nd_191_w7pjeta_perform_write_launch_analysis $opcname $inst pins var summary

   #######################################
   # Address/command Analyses

   mem0_altera_emif_arch_nd_191_w7pjeta_perform_ac_analysis  $opcname $inst pins var summary

   #######################################
   # DQS Gating Analysis

   mem0_altera_emif_arch_nd_191_w7pjeta_perform_dqs_gating_analysis   $opcname $inst pins var summary

   #######################################
   # Write Levelling Analysis

   mem0_altera_emif_arch_nd_191_w7pjeta_perform_write_levelling_analysis $opcname $inst pins var summary

   #######################################
   # Analyses related to FPGA core

   mem0_altera_emif_arch_nd_191_w7pjeta_perform_core_analysis $opcname $inst pins var summary

   #######################################
   # Print out the Summary Panel for this instance

   set summary [lsort -command mem0_altera_emif_arch_nd_191_w7pjeta_sort_proc $summary]

   set f -1
   set fname "${fbasename}_summary.csv"

   if { [mem0_altera_emif_arch_nd_191_w7pjeta_get_operating_conditions_number] == 0 } {
      set f [open $fname w]

      puts $f "Core: ${::GLOBAL_mem0_altera_emif_arch_nd_191_w7pjeta_corename} - Instance: $inst"
   ...
   post_message -type info "Core: ${::GLOBAL_mem0_altera_emif_arch_nd_191_w7pjeta_corename} - Instance: $inst"
   ...
   foreach summary_line $summary {
      ...
      if {$su != "--"} {
         set su [ mem0_altera_emif_arch_nd_191_w7pjeta_round_3dp $su]
      }
      if {$hold != "--"} {
         set hold [ mem0_altera_emif_arch_nd_191_w7pjeta_round_3dp $hold]
      }
