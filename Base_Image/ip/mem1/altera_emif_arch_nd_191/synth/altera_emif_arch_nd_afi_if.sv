...
`define _l2core_afi(_loc, _port_i, _phase_i) l2core_data\
   ...
   [`_get_lane(_loc, _port_i)]\
   [(`_get_pin(_loc, _port_i) * 8) + _phase_i]
`define _l2core_afi_sdr(_loc, _port_i, _phase_i) l2core_data\
   [`_get_tile(_loc, _port_i)]\
   [`_get_lane(_loc, _port_i)]\
   [(`_get_pin(_loc, _port_i) * 8) + (_phase_i * 2)]
...
`define _connect_in_sdr(_loc, _mem_port_width, _afi_port_width, _afi_port) \
   for (port_i = 0; port_i < _mem_port_width; ++port_i) begin : iport \
      for (phase_i = 0; phase_i < _afi_port_width / _mem_port_width; ++phase_i) begin : data_phase \
         assign _afi_port[_mem_port_width * phase_i + port_i] = `_l2core_afi_sdr(_loc, port_i, phase_i); \
      end \
   end  
...
`define _connect_in_sdr_with_regs(_loc, _mem_port_width, _afi_port_width, _afi_port) \
   logic [_afi_port_width-1:0] sr_i; \
   `_connect_in_sdr(_loc, _mem_port_width, _afi_port_width, sr_i) \
   altera_emif_arch_nd_regs # ( \
      .REGISTER     (REGISTER_AFI_P2C), \
      .WIDTH        (_afi_port_width) \
   ) afi_regs_i ( \
   ...
   generate
      ...
      if (`_get_pin_count(PORT_MEM_DQA_PINLOC) != 0 && `_get_pin_count(PORT_MEM_DQB_PINLOC) != 0) begin : dual_port
         ...
         begin
            ...
            for (tile_i = 0; tile_i < NUM_OF_RTL_TILES; ++tile_i) begin : tile
               for (lane_i = 0; lane_i < LANES_PER_TILE; ++lane_i) begin : lane
                  if (`_is_ac_lane(tile_i, lane_i)) begin
                     assign core2l_rdata_en_full[tile_i][lane_i] = 4'b1111;
                  end else begin
                     assign core2l_rdata_en_full[tile_i][lane_i] = afi_rdata_en_full_r_padded;
                  end 
               end
            end
      ...
      if (`_get_pin_count(PORT_MEM_GNT_N_PINLOC) != 0) begin : mem_gnt_n
         `_connect_out_with_regs(PORT_MEM_GNT_N_PINLOC, PORT_MEM_GNT_N_WIDTH, PORT_AFI_GNT_N_WIDTH, afi_gnt_n, OE_ON_WIDTH, oe_on)
      end
      ...
      if (`_get_pin_count(PORT_MEM_REQ_N_PINLOC) != 0) begin : mem_req_n
         `_connect_in_sdr_with_regs(PORT_MEM_REQ_N_PINLOC, PORT_MEM_REQ_N_WIDTH, PORT_AFI_REQ_N_WIDTH, afi_req_n)
         
         logic [PORT_MEM_REQ_N_WIDTH-1:0] zeros;
         assign zeros = '0;
         // Switching OE on read pins as we switch OE_INVERT in the IP
         `_connect_out(PORT_MEM_REQ_N_PINLOC, PORT_MEM_REQ_N_WIDTH, PORT_MEM_REQ_N_WIDTH, zeros, OE_ON_WIDTH, oe_on)
      end else begin : no_mem_req_n
         assign afi_req_n = '0;
      end

      if (`_get_pin_count(PORT_MEM_ERR_N_PINLOC) != 0) begin : mem_err_n
         `_connect_in_sdr_with_regs(PORT_MEM_ERR_N_PINLOC, PORT_MEM_ERR_N_WIDTH, PORT_AFI_ERR_N_WIDTH, afi_err_n)
         
         logic [PORT_MEM_ERR_N_WIDTH-1:0] zeros;
         assign zeros = '0;
         // Switching OE on read pins as we switch OE_INVERT in the IP
         `_connect_out(PORT_MEM_ERR_N_PINLOC, PORT_MEM_ERR_N_WIDTH, PORT_MEM_ERR_N_WIDTH, zeros, OE_ON_WIDTH, oe_on)
      end else begin : no_mem_err_n
         assign afi_err_n = '0;
      end
