...
source "$script_dir/mem1_altera_emif_arch_nd_191_olgykcq_utils.tcl"

load_package sdc_ext

proc mem1_altera_emif_arch_nd_191_olgykcq_get_ddr_pins { instname allpins var_array_name} {
   ...
   set pins(pll_c0_periph_clock) [mem1_altera_emif_arch_nd_191_olgykcq_sort_duplicate_names $pins(pll_c0_periph_clock)]
   ...
   set pins(pll_c1_periph_clock) [mem1_altera_emif_arch_nd_191_olgykcq_sort_duplicate_names $pins(pll_c1_periph_clock)]
   ...
   set pins(vco_clock) [mem1_altera_emif_arch_nd_191_olgykcq_sort_duplicate_names $pins(vco_clock)]
   ...
   if {$var(IS_HPS)} {
      ...
      for {set cpa_idx 0} {$cpa_idx < $num_of_cpa_blocks} {incr cpa_idx} {
         ...
         set core_reset_sync_clock_id [mem1_altera_emif_arch_nd_191_olgykcq_get_output_clock_id $sync_reset_reg "Usr clock" msg_list var]
         if {$core_reset_sync_clock_id == -1} {
            foreach {msg_type msg} $msg_list {
               post_message -type $msg_type "mem1_altera_emif_arch_nd_191_olgykcq_pin_map.tcl: $msg"
            }
            post_message -type error "mem1_altera_emif_arch_nd_191_olgykcq_pin_map.tcl: Failed to find clock source for register $sync_reset_reg"

            if {$var(PHY_CORE_CLKS_SHARING_ENUM) == "CORE_CLKS_SHARING_SLAVE"} {
               post_message -type error "mem1_altera_emif_arch_nd_191_olgykcq_pin_map.tcl: This is a clock sharing SLAVE interface. Please ensure that the clks_sharing_master_out port of the master is connected to the clks_sharing_master_in port of the slave(s)."
               if {$cpa_idx > 0} {
                  post_message -type error "mem1_altera_emif_arch_nd_191_olgykcq_pin_map.tcl: This clock sharing slave interface uses a Ping-Pong PHY and has extra clock/reset requirements. Please ensure that the master interface is also a ping-pong interface. A ping-pong interface can act as clock sharing master for both ping-pong and non-ping-pong interfaces."
               }
            } else {
               post_message -type error "mem1_altera_emif_arch_nd_191_olgykcq_pin_map.tcl: Please ensure that the register has not been removed or optimized away."
            }
         } else {
            set core_reset_sync_clock [mem1_altera_emif_arch_nd_191_olgykcq_get_pll_clock_name $core_reset_sync_clock_id]
         }
         ...
         if {[regexp {(^.*)\|arch\|arch_inst\|io_tiles_wrap_inst\|io_tiles_inst\|tile_gen\[([0-9])\].tile_ctrl_inst(.*)\|pa_core_clk_out\[[0-9]\]$} $core_reset_sync_clock matched pins(master_instname) tilegen_num tile_instnum] == 1} {
            ...
            if { $::TimeQuestInfo(nameofexecutable) == "quartus_map" || $::TimeQuestInfo(nameofexecutable) == "quartus_syn"} {
               ...
               set vco_clock_id [mem1_altera_emif_arch_nd_191_olgykcq_get_vco_clk_id $core_reset_sync_clock_id var]
            ...
            post_message -type error "mem1_altera_emif_arch_nd_191_olgykcq_pin_map.tcl: Failed to find CPA outputs."
         }
      }
      ...
      set var(pll_c3_cnt) [mem1_altera_emif_arch_nd_191_olgykcq_calculate_counter_value $var(PLL_C_CNT_HIGH_3) $var(PLL_C_CNT_LOW_3) $var(PLL_C_CNT_BYPASS_EN_3)]
      set pins(master_cal_slave_clk) "$pll_master_user_clock_base\[3\]"

      set var(pll_c4_cnt) [mem1_altera_emif_arch_nd_191_olgykcq_calculate_counter_value $var(PLL_C_CNT_HIGH_4) $var(PLL_C_CNT_LOW_4) $var(PLL_C_CNT_BYPASS_EN_4)]
   ...
   set pll_ref_clock_id [mem1_altera_emif_arch_nd_191_olgykcq_get_input_clk_id $pins(pll_cascade_in_id) var]
   if {$pll_ref_clock_id == -1} {
      post_message -type critical_warning "mem1_altera_emif_arch_nd_191_olgykcq_pin_map.tcl: Failed to find PLL reference clock"
   } else {
   ...
   foreach {pin_type pattern} $patterns {
      if {[string match "*|o" $pattern]} {
         set local_pins [ mem1_altera_emif_arch_nd_191_olgykcq_get_names_in_collection [ get_fanouts $pattern ] ]
      } else {
         set local_pins [ mem1_altera_emif_arch_nd_191_olgykcq_get_names_in_collection [ get_fanins $pattern ] ]
      }
   ...
   if {$var(PLL_NUM_OF_EXTRA_CLKS) > 0} {
      ...
      for {set i 0} {$i < $var(PLL_NUM_OF_EXTRA_CLKS)} {incr i} {
         ...
         set var(pll_c${i_cnt_num}_cnt) [mem1_altera_emif_arch_nd_191_olgykcq_calculate_counter_value $var(PLL_C_CNT_HIGH_${i_cnt_num}) $var(PLL_C_CNT_LOW_${i_cnt_num}) $var(PLL_C_CNT_BYPASS_EN_${i_cnt_num})]
         set pins(pll_extra_clk_${i}) "$pll_master_user_clock_base\[$i_cnt_num\]"
      }
   }
}

proc mem1_altera_emif_arch_nd_191_olgykcq_initialize_ddr_db { ddr_db_par var_array_name} {
   upvar $ddr_db_par local_ddr_db
   upvar 1 $var_array_name var

   global ::GLOBAL_mem1_altera_emif_arch_nd_191_olgykcq_corename
   global ::io_only_analysis

   post_sdc_message info "Initializing DDR database for CORE $::GLOBAL_mem1_altera_emif_arch_nd_191_olgykcq_corename"
   set instance_list [mem1_altera_emif_arch_nd_191_olgykcq_get_core_instance_list $::GLOBAL_mem1_altera_emif_arch_nd_191_olgykcq_corename]

   foreach instname $instance_list {

      if {$::io_only_analysis == 0}  {
         post_sdc_message info "Finding port-to-pin mapping for CORE: $::GLOBAL_mem1_altera_emif_arch_nd_191_olgykcq_corename INSTANCE: $instname"
         mem1_altera_emif_arch_nd_191_olgykcq_get_ddr_pins $instname allpins var
         mem1_altera_emif_arch_nd_191_olgykcq_verify_ddr_pins allpins var
      }
...
proc mem1_altera_emif_arch_nd_191_olgykcq_verify_ddr_pins { pins_par var_array_name} {
...
proc mem1_altera_emif_arch_nd_191_olgykcq_get_all_instances_dqs_pins { ddr_db_par } {
   upvar $ddr_db_par local_ddr_db
...
proc mem1_altera_emif_arch_nd_191_olgykcq_calculate_counter_value { cnt_hi cnt_lo cnt_bypass } {
   if {$cnt_bypass} {
      set result 1
   } else {
...
proc mem1_altera_emif_arch_nd_191_olgykcq_get_input_clk_id { pll_inclk_id var_array_name} {
   upvar 1 $var_array_name var

   array set results_array [list]

   mem1_altera_emif_arch_nd_191_olgykcq_traverse_fanin_up_to_depth $pll_inclk_id mem1_altera_emif_arch_nd_191_olgykcq_is_node_type_pin clock results_array $var(pll_inclock_search_depth)
...
proc mem1_altera_emif_arch_nd_191_olgykcq_get_output_clock_id { pin_list pin_type msg_list_name var_array_name} {
   ...
   mem1_altera_emif_arch_nd_191_olgykcq_get_pll_clock $output_id_list $pin_type output_clock_id $var(pll_outclock_search_depth)
   return $output_clock_id
}

proc mem1_altera_emif_arch_nd_191_olgykcq_get_pll_clock { dest_id_list node_type clock_id_name search_depth} {
   if {$clock_id_name != ""} {
   ...
   foreach node_id $dest_id_list {
      mem1_altera_emif_arch_nd_191_olgykcq_traverse_fanin_up_to_depth $node_id mem1_altera_emif_arch_nd_191_olgykcq_is_node_type_pll_clk clock clk_array $search_depth
   }
...
proc mem1_altera_emif_arch_nd_191_olgykcq_get_vco_clk_id { wf_clock_id var_array_name} {
   upvar 1 $var_array_name var

   array set results_array [list]

   mem1_altera_emif_arch_nd_191_olgykcq_traverse_fanin_up_to_depth $wf_clock_id mem1_altera_emif_arch_nd_191_olgykcq_is_node_type_vco clock results_array $var(pll_vcoclock_search_depth)
   if {[array size results_array] == 1} {
      set pin_id [lindex [array names results_array] 0]
      set result $pin_id
   } else {
...
proc mem1_altera_emif_arch_nd_191_olgykcq_is_node_type_pll_clk { node_id } {
...
proc mem1_altera_emif_arch_nd_191_olgykcq_is_node_type_vco { node_id } {
   set cell_id [get_node_info -cell $node_id]
...
proc mem1_altera_emif_arch_nd_191_olgykcq_does_ref_clk_exist { ref_clk_name } {
...
proc mem1_altera_emif_arch_nd_191_olgykcq_get_p2c_c2p_clock_uncertainty { instname var_array_name } {
   ...
   if {$read_atom_netlist_error == 0} {
      if {[mem1_altera_emif_arch_nd_191_olgykcq_are_entity_names_on]} {
      ...
      if {$success == 1} {
         ...
         set cp_current [mem1_altera_emif_arch_nd_191_olgykcq_get_cp_current_from_setting $cp_setting]
         ...
         set SPE  [mem1_altera_emif_arch_nd_191_olgykcq_get_spe_from_cp_current $cp_current]

         if {$success == 1} {
            set clock_uncertainty_sqrt  [expr sqrt(($LFD/2)*($LFD/2) + ($LFD/2)*($LFD/2))]
            set clock_uncertainty [mem1_altera_emif_arch_nd_191_olgykcq_round_3dp [expr ($clock_uncertainty_sqrt + $SPE)*1e9]]
...
proc mem1_altera_emif_arch_nd_191_olgykcq_get_cp_current_from_setting { cp_setting } {
...
proc mem1_altera_emif_arch_nd_191_olgykcq_get_spe_from_cp_current { cp_current } {
...
proc mem1_altera_emif_arch_nd_191_olgykcq_get_periphery_clock_uncertainty { results_array_name var_array_name } {
...
proc mem1_altera_emif_arch_nd_191_olgykcq_get_core_clock_uncertainty { results_array_name var_array_name } {
   upvar 1 $results_array_name results
   upvar 1 $var_array_name var
...
proc mem1_altera_emif_arch_nd_191_olgykcq_get_core_overconstraints { results_array_name var_array_name } {
   upvar 1 $results_array_name results
   upvar 1 $var_array_name var
...
proc mem1_altera_emif_arch_nd_191_olgykcq_get_periphery_overconstraints { results_st_array_name results_mt_array_name var_array_name } {
...
proc mem1_altera_emif_arch_nd_191_olgykcq_sort_duplicate_names { names_array } {
