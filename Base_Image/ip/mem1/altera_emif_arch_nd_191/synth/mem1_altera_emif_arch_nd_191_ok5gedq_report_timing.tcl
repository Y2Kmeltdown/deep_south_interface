...
if { ! [timing_netlist_exist] } {
   ...
   source "$script_dir/mem1_altera_emif_arch_nd_191_ok5gedq_report_timing_core.tcl"
   if { ! [timing_netlist_exist] } {
...
mem1_altera_emif_arch_nd_191_ok5gedq_initialize_ddr_db ddr_db var
...
foreach inst $instances {
   ...
   if {[llength $instances] <= 1} {
      set fbasename "${::GLOBAL_mem1_altera_emif_arch_nd_191_ok5gedq_corename}"
   } else {
      set fbasename "${::GLOBAL_mem1_altera_emif_arch_nd_191_ok5gedq_corename}_${inst_id}"
   }
   ...
   mem1_altera_emif_arch_nd_191_ok5gedq_perform_read_capture_analysis $opcname $inst pins var summary

   #######################################
   # Write Analysis

   mem1_altera_emif_arch_nd_191_ok5gedq_perform_write_launch_analysis $opcname $inst pins var summary

   #######################################
   # Address/command Analyses

   mem1_altera_emif_arch_nd_191_ok5gedq_perform_ac_analysis  $opcname $inst pins var summary

   #######################################
   # DQS Gating Analysis

   mem1_altera_emif_arch_nd_191_ok5gedq_perform_dqs_gating_analysis   $opcname $inst pins var summary

   #######################################
   # Write Levelling Analysis

   mem1_altera_emif_arch_nd_191_ok5gedq_perform_write_levelling_analysis $opcname $inst pins var summary

   #######################################
   # Analyses related to FPGA core

   mem1_altera_emif_arch_nd_191_ok5gedq_perform_core_analysis $opcname $inst pins var summary

   #######################################
   # Print out the Summary Panel for this instance

   set summary [lsort -command mem1_altera_emif_arch_nd_191_ok5gedq_sort_proc $summary]

   set f -1
   set fname "${fbasename}_summary.csv"

   if { [mem1_altera_emif_arch_nd_191_ok5gedq_get_operating_conditions_number] == 0 } {
      set f [open $fname w]

      puts $f "Core: ${::GLOBAL_mem1_altera_emif_arch_nd_191_ok5gedq_corename} - Instance: $inst"
   ...
   post_message -type info "Core: ${::GLOBAL_mem1_altera_emif_arch_nd_191_ok5gedq_corename} - Instance: $inst"
   ...
   foreach summary_line $summary {
      ...
      if {$su != "--"} {
         set su [ mem1_altera_emif_arch_nd_191_ok5gedq_round_3dp $su]
      }
      if {$hold != "--"} {
         set hold [ mem1_altera_emif_arch_nd_191_ok5gedq_round_3dp $hold]
      }
